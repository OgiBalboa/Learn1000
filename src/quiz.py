...
class Quiz():
    def __init__(self, db, udb, wordgroup):
        ...
        if self.wordgroup is "auto":
            self.wordlist, \
                ...
                self.tablist, \
                self.rowidlist, \
                self.reglist = auto_generate(udb_=self.udb, db_=self.db,
                                             param="quiz")
        else:
            self.table = wordgroup
            self.wordlist, self.rowidlist, self.reglist = get_words(self.db,
                                                                    self.table)
            self.trlist = None
    ...
    def pressed(self, ans):
        self.search = check_word(db_=self.udb, table=self.table,
                                 rowid=self.rowid)
        if self.search == None:
    ...
    def true(self, ):
        ...
        self.udb.update(self.table, rowid=self.rowid, inc_tcount=1,
                        inc_fcount=0)
    ...
    def false(self, ):
        ...
        self.fls += 1
        self.udb.update(self.table, rowid=self.rowid, inc_tcount=0,
                        inc_fcount=1)
    ...
    def new_word(self):
        if self.wordgroup != "auto":
            self.word, self.translation, self.rowid, self.reg = random_word(
                wordlist=self.wordlist,
                rowidlist=self.rowidlist,
                reglist=self.reglist)
        else:
            self.word, self.translation, self.rowid, self.table, self.reg = random_word(
                wordlist=self.wordlist,
                reglist=self.reglist,
                rowidlist=self.rowidlist, \
                trlist=self.trlist,
