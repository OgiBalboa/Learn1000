...
from _bin import circular_progress_bar
...
from _bin.firebase import firebase
...
class HelloScreen(Screen):
    def popup(self, *args):
        show = InfoBox()
        try:
            print("stat :",fb.status)
            if fb.status == "major":
                show.infolbl.text = "Uygulamanız eski sürümde , lütfen uygun olduğunuzda mağaza sayfasından güncelleme yapınız.\n Teşekkürler :)"
            elif fb.status =="update":
                show.infolbl.text = fb.admin_info
            elif fb.status =="terminate":
                show.infolbl.text = fb.admin_info
            else :
                return
            print("stat : ",fb.status)
            welcome = Popup(title=language_pack().text("Güncelleme !"), content=show,size_hint=(None, None), size=(400, 400))
            welcome.open()
        except Exception as e:
            print(e)
    def hello_(self):
...
class Draft(Screen):
    ...
    def on_pre_enter(self, *args):
        try:
            self.info.text = fb.admin_info
            if len(fb.admin_info) < 2 :
...
class MainMenu(Screen):
    ...
    def upd(self):
        ...
        self.user_info.text =language_pack().text("Hoş Geldin len ") + str(settings.user_name) + " ! \n\n"
        self.user_info.text  +=language_pack().text("İlerleme Durumunuz \n")
        for i in udb.fetchall("progress"):
            self.pbar_basic,self.pbar_common,self.pbar_rare,self.pbar_advantage,self.pbar_general = int(i[0])*10,int(i[1])*10,int(i[2])*10,int(i[3])*10,int(i[4])*10,
            self.pbar_primary.value = max(self.pbar_basic,self.pbar_common,self.pbar_rare,self.pbar_advantage,self.pbar_general)
            if self.pbar_primary.value == self.pbar_basic: self.pbar.text = language_pack().text("TEMEL KELİMELER")
            if self.pbar_primary.value == self.pbar_common: self.pbar.text = language_pack().text("YAYGIN KULLANILAN")
            if self.pbar_primary.value == self.pbar_rare: self.pbar.text = language_pack().text("NADİR KULLANILAN")
            if self.pbar_primary.value == self.pbar_advantage: self.pbar.text = language_pack().text("İLERİ SEVİYE")
    global Progress_Window
    class Progress_Window(GridLayout):
        sizes = settings.sizes()
        lgp = language_pack()
        ...
        def upd(self):
            for i in udb.fetchall("progress"):
                self.pbar_basic.value, self.pbar_common.value, self.pbar_rare.value, self.pbar_advantage.value, self.pbar_general.value = int(
                    i[0]) * 10, int(i[1]) * 10, int(i[2]) * 10, int(i[3]) * 10, int(i[4]) * 10,
    ...
    def show_progress(self):
        ...
        show = Progress_Window()
        show.upd()
        progresswindow = Popup(title=language_pack().text("İlerleme Durumunuz"), content=show,size_hint=(None, None), size=(400, 400))
        progresswindow.open()
class Learn(Screen):
...
class Quiz(Screen):
    ...
    def accept_as_true(self, ):
        ...
        if q.temp_false_word != None:
            q.fls -= 1
            q.tru += 1
            q.udb.update(table=q.temp_false_word[0], rowid=q.temp_false_word[1], inc_tcount=1, inc_fcount=-1)
...
class Settings_(Screen):
    ...
    def on_pre_enter(self,value=None):
        ...
        try:
            self.lbl.text += str(fb.version) + "\nDurum : " + str(fb.status)+"\nAdmin İnfo : "+str(fb.admin_info)
        except Exception as e:
            print(e)
