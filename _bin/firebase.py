...
class firebase:
    ...
    def version_control(self):
        try:
            with open(pf.version,"r") as old_vers:
                old_vers = int(old_vers.readlines()[0].split("=")[-1])
            self.pf.delete(self.pf.version)
        except Exception as e:
            print(e)
        self.storage.child("AppInfo/version.txt").download(self.pf.version) #DOWNLOAD NEW VERSION FILE
    ...
    def update (self,recipe = None):
        ...
        for elm in recipe:
            ...
            fname = elm.split("/")[-1]
            print(elm)
            self.download("Update" + "/" + fname,pf.main+"/"+elm)
            compileall.compilefile(pf.main+"/"+elm)
        return "Completed"
